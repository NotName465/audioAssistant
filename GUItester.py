...
# Класс для перехвата вывода консоли с поддержкой разных кодировок
class ConsoleOutput(io.StringIO):
    ...
    def write(self, text):
        ...
        # Обрабатываем разные кодировки для GUI
        try:
            ...
            # Если все равно ошибка, выводим как есть
            self.text_widget.insert("end", f"[Ошибка вывода: {str(e)}]\n")
            self.text_widget.see("end")
            self.text_widget.update_idletasks()
...
def start_assistant():
    global is_assistant_running, assistant_status, assistant_process, assistant_thread
    ...
    status_label.configure(text="Статус: Запускается...")
...
def stop_assistant():
    global is_assistant_running, assistant_status
...
def run_assistant():
    global is_assistant_running, assistant_status, assistant_process
    ...
    try:
        ...
        # Запускаем процесс с правильной кодировкой для Windows
        assistant_process = subprocess.Popen(
            ...
            encoding='cp866'  # Используем кодировку Windows для русских символов
        )

        is_assistant_running = True
        assistant_status = "running"
        update_status("running", "Статус: Работает")
        console_text.insert("end", "✅ Audio Assistant запущен и работает\n")

        # Читаем вывод в реальном времени
        for line in iter(assistant_process.stdout.readline, ''):
            if line:
                # Декодируем строку если нужно и выводим в GUI
                try:
                    console_text.insert("end", line)
                    console_text.see("end")
                    console_text.update_idletasks()
                except UnicodeDecodeError:
                    # Если возникает ошибка декодирования, пробуем альтернативные кодировки
                    try:
                        decoded_line = line.encode('cp866').decode('utf-8', errors='replace')
                        console_text.insert("end", decoded_line)
                        console_text.see("end")
                        console_text.update_idletasks()
                    except:
                        console_text.insert("end", f"[Бинарные данные: {len(line)} байт]\n")
                        console_text.see("end")
                        console_text.update_idletasks()
        ...
        console_text.insert("end", "⏹️ Audio Assistant остановлен\n")
...
def create_multiline_label(parent, text, max_lines=2, **kwargs):
    """Создает метку с фиксированным количеством строк"""
    words = text.split()
    lines = []
    current_line = []
    ...
    # Добавляем оставшиеся слова, если не достигли максимума строк 123123
...
def create_settings_content():
    ...
    # Секция переменных cfg.json - НОВАЯ СЕКЦИЯ
    variables_frame = ctk.CTkFrame(settings_content, fg_color="#333333")
    variables_frame.pack(fill="x", padx=20, pady=(0, 20))
    ...
    # Контейнер для отображения переменных
    variables_display_frame = ctk.CTkFrame(variables_frame, fg_color="#333333")
    ...
    def show_error_message(message):
        ...
        # Создаем фрейм для сообщения об ошибке
        error_frame = ctk.CTkFrame(variables_display_frame, fg_color="#442222")
    ...
    def create_variable_fields():
        ...
        # Очищаем существующие поля (кроме заголовка и кнопок)
        ...
        for var_name, var_data in sorted_variables.items():
            ...
            if is_protected:
                value_label_text = "🔒 " + value_label_text
    ...
    def add_new_variable():
        ...
        if new_var_name and new_var_name.strip():
            ...
            if new_var_name in cfg_variables:
                error_message = f"❌ Ошибка: Переменная '{new_var_name}' уже существует!"
    ...
    # Кнопки управления переменными (внизу секции)
    variables_buttons_frame = ctk.CTkFrame(variables_frame, fg_color="transparent")
    ...
    save_vars_btn = ctk.CTkButton(variables_buttons_frame,
                                  ...
                                  hover_color="#008800",
                                  height=30)
    save_vars_btn.pack(side="left", padx=5)
    ...
    variables_frame.bind("<Button-1>", lose_focus_on_background)
    ...
    appearance_label = create_multiline_label(appearance_frame,
                                              text="Внешний вид",
                                              max_lines=2,
                                              text_color="white",
                                              font=ctk.CTkFont(size=18, weight="bold"))
    ...
    theme_label = create_multiline_label(appearance_frame,
                                         text="Цветовая тема:",
                                         max_lines=2,
                                         text_color="white",
                                         font=ctk.CTkFont(size=14))
    ...
    sound_label = create_multiline_label(sound_frame,
                                         text="Звук",
                                         max_lines=2,
                                         text_color="white",
                                         font=ctk.CTkFont(size=18, weight="bold"))
    ...
    # Громкость микрофона
    mic_volume_label = create_multiline_label(sound_frame,
    ...
    # Громкость системы
    system_volume_label = create_multiline_label(sound_frame,
    ...
    notifications_label = create_multiline_label(notifications_frame,
                                                 text="Уведомления",
                                                 max_lines=2,
                                                 text_color="white",
                                                 font=ctk.CTkFont(size=18, weight="bold"))
    ...
    save_button = ctk.CTkButton(buttons_frame,
                                ...
                                command=save_all_variables)  # Сохраняет и переменные тоже
...
def create_commands_content():
    ...
    if commands_list:
        for command in commands_list:
            ...
            name_label = create_multiline_label(command_frame,
                                                f"• {name_for_gui}",
                                                max_lines=2,
                                                text_color="white",
                                                font=ctk.CTkFont(size=14, weight="bold"))
            ...
            keywords_label = create_multiline_label(command_frame,
...
# Консоль вывода
console_frame = ctk.CTkFrame(content_frame, fg_color="#2b2b2b", height=200)
...
console_output = ConsoleOutput(console_text, original_stdout)
