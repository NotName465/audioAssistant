...
def run_assistant():
    ...
    try:
        ...
        # Читаем вывод в реальном времени с обработкой кодировки
        while True:
            line = assistant_process.stdout.readline()
            if not line:
                break

            # Обрабатываем строку для корректного отображения
            try:
        ...
        if return_code == 0:
            ...
            update_status("error", f"Статус: Ошибка (код: {return_code})")
